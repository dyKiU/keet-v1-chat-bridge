...
test("summarizeKeetChatMessages extracts text from Keet chat shapes", () => {
  ...
  assert.deepEqual(messages, [
    ...
    { seq: 3, timestamp: undefined, author: "llm", text: "[llm] hi" },
  ]);
  assert.equal(newestSeq(messages), 3);
});

test("shouldReply ignores assistant replies and thinking markers", () => {
  assert.equal(shouldReply({ seq: 1, text: "what model are you?" }), true);
  assert.equal(shouldReply({ seq: 2, text: "[llm] qwen3-4b" }), false);
  assert.equal(shouldReply({ seq: 6, text: "[qvac] legacy prefix" }), false);
...
test("collectAnswer does not show thinking when v1 chat server fails before streaming", async () => {
  let thinkingCount = 0;
...
test("collectAnswer shows thinking after v1 chat server accepts quick responses", async () => {
  let thinkingCount = 0;
...
test("collectAnswer shows thinking while waiting on a live v1 chat response", async () => {
  let thinkingCount = 0;
